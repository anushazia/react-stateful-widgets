...
export default function Programmers() {
  ...
  const [programmerName, setProgramerName] = useState('')
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: programmerName == true ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfProgrammers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => setProgramerName(dev.id)}>Feature</button>
            </div>
          )
        }
      </div>
